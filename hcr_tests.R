# plot biomass vs exploitation (catch / biomass) for each run
library(tidyverse)

all_runs <- c(1914,2014,2015)
#all_runs <- c(1906:1909, 1934:1937, 1938:1941)
#all_runs <- c(1906:1909, 1942:1945, 1934:1937, 1946:1949)

# Plot total OY catch -----------------------------------------------------
# this_cap <- rep((c(400,300,200,100)*1000),3)
# this_cap <- c(rep(c(300)*1000,4), rep(c(200)*1000,4))
# this_cap <- c("No cap","100,000")
#oy_species <- c("COD","POL","POP","FFS","ATF")
this_cap <- rep(NA,3)
oy_species <- "POL"
...
key_wgts <- data.frame("wgts" = rep(NA,3),#,
  #rep("10,10,10,1,1",4)), # this was supposed to be the opposite but this shows us what happens
  #rep("1000,100,10,0.1,0.01", 4)),
  "run" = all_runs)

key_gamma <- data.frame("gamma" = c(0.25,0.5,0.75),#,
                       "run" = all_runs)

key_conf <- data.frame("conf" = c("Base","RecDev","RecDev+noCrash"),#,
                      "run" = all_runs)

catch_df <- catch_df %>% left_join(key_wgts) %>% left_join(key_gamma) %>% left_join(key_conf)

catch_df$wgts <- factor(catch_df$wgts, levels = c("Equal","Binary","Value-based"))

# aff climate key if relevant
# climate_key <- data.frame("clim" = rep(c(
#   rep("1999",4),
#   rep("2100_ssp245",4)), 2), 
#   "run" = all_runs)
# catch_df <- catch_df %>% left_join(climate_key)
